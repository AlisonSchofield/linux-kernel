...
unsigned int nd_region_acquire_lane(struct nd_region *nd_region)
{
	...
	/*
	 * Since migrate_disable() allows preemption, multiple tasks may
	 * execute on the same CPU. Use @owner to distinguish recursive
	 * acquisition by the same task from different tasks sharing the
	 * CPU. Only same-task recursion bypasses the spinlock.
	 */
...
void nd_region_release_lane(struct nd_region *nd_region, unsigned int lane)
{
	...
	WARN_ON_ONCE(READ_ONCE(ndl->owner) != current);
	WARN_ON_ONCE(ndl->count <= 0);
	if (--ndl->count == 0) {
